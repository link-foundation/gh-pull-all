...
async function testMultiThreadNoLive() {
  const testDir = path.join(os.tmpdir(), 'gh-pull-all-test-multithread-no-live')
  ...
  try {
    ...
    const result = execSync(`../gh-pull-all.mjs --user octocat --threads 3 --no-live-updates --dir ${testDir}`, {
      encoding: 'utf8',
      stdio: 'pipe'
    })
